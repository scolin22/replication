...
public class Backup {
    private static final Map<Integer, Map<ByteKey, byte[]>> backups = new ConcurrentHashMap<>();
    ...
    public static synchronized void pruneReplicates() {
        List<Integer> replicateIDs = Router.getReplicateClientIDs(Router.getMyNode()); // Supposed to have
        Set<Integer> backups = Backup.getBackupList(); // Currently have
        for (Integer backupID : backups) {
    ...
    public static synchronized boolean put(Integer backupID, ByteKey key, byte[] value) {
